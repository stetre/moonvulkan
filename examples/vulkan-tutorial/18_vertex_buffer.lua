...
local vert_shader = vk.create_shader_module(device, 0, glsl_to_spirv("shaders/vertexbuffer.vert"))
local frag_shader = vk.create_shader_module(device, 0, glsl_to_spirv("shaders/vertexbuffer.frag"))
...
do 
   -- 1) Create buffer
   vertex_buffer = vk.create_buffer(device, {
      size = #vertexdata,
   ...
   local reqs = vk.get_buffer_memory_requirements(vertex_buffer)
   local flags = vk.MEMORY_PROPERTY_HOST_VISIBLE_BIT | vk.MEMORY_PROPERTY_HOST_COHERENT_BIT
   local mti = find_memory_type(physdev, reqs, flags)
   vertex_buffer_memory = vk.allocate_memory(device, reqs.size, mti)
   vk.bind_buffer_memory(vertex_buffer, vertex_buffer_memory, 0)

   -- 3) Copy the vertex data to the buffer
   vk.map_memory(vertex_buffer_memory, 0, #vertexdata)
   vk.write_memory(vertex_buffer_memory, 0, vertexdata)
   vk.unmap_memory(vertex_buffer_memory)
end
