...
do 
   ...
   info.layout = vk.create_pipeline_layout(device, 0, nil, nil)
   info.render_pass = render_pass
   info.subpass = 0

   graphics_pipeline = vk.create_graphics_pipelines(device, nil, {info})[1]

   vk.destroy_shader_module(vert_shader)
   vk.destroy_shader_module(frag_shader)
end

-- Create framebuffers
local swapchain_framebuffers = {}
...
local command_buffers = vk.allocate_command_buffers(command_pool, 'primary', #swapchain_framebuffers)
...
while not glfw.window_should_close(window) do
   ...
   local image_index = assert(vk.acquire_next_image(swapchain, 'blocking', image_available_sem))
   ...
   vk.queue_present(present_queue, {
