...
_current_extent_: <<extent2d, extent2d>> (may be _nil_, see Note1 below), +
...
_shared_present_supported_usage_flags_: <<imageusageflags, imageusageflags>> (see Note2 below), +
...
Note1: The _current_extent_ field is not set when _width_ and _height_ have the special value 0xFFFFFFFF, meaning that the current extent is undefined. +
Note2: This field is present only if the relevant extensions are enabled (rfr: https://www.khronos.org/registry/vulkan/specs/1.0-extensions/html/vkspec.html#VkSharedPresentSurfaceCapabilitiesKHR[VkSharedPresentSurfaceCapabilitiesKHR]).#
