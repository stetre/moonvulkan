...
_format_: <<format, format>>, +
_extent_: <<extent3d, extent3d>>, +
_mip_levels_: integer or '_remaining_' (defaults to 1), +
_array_layers_: integer or '_remaining_' (defaults to 1), +
_samples_: <<samplecountflags, samplecountflags>> (defaults to 1), +
_tiling_: <<imagetiling, imagetiling>>, +
_usage_: <<imageusageflags, imageusageflags>>, +
_initial_layout_: <<imagelayout, imagelayout>>, +
...
_mag_filter_, _min_filter_: <<filter, filter>>, +
_mipmap_mode_: <<samplermipmapmode, samplermipmapmode>>, +
_address_mode_u/v/w_: <<sampleraddressmode, sampleraddressmode>>, +
...
_viewport_count_: integer, +
_scissor_count_: integer, +
...
_src_access_mask_: <<accessflags, accessflags>>, +
_dst_access_mask_: <<accessflags, accessflags>>, +
_src_queue_family_index_: integer, +
_dst_queue_family_index_: integer, +
_buffer_: <<buffer, buffer>> *[R]*, +
_offset_: integer, +
_size_: integer, +
...
[small]#*viewport* = { +
_x_, _y_: float, +
_width_, _height_: float, +
_min_depth_, _max_depth_: float, +
...
_presentmode_: <<presentmode, presentmode>>, +
_clipped_: boolean, +
...
_wait_semaphores_: {<<semaphore, semaphore>>} (opt.), +
